...
private theorem inner_genQuot_eq (S : ContractionSemigroup X)
    (hS : S.toStronglyContinuousSemigroup.IsComplexLinear)
    (hself : ∀ t, IsSelfAdjoint (S.toStronglyContinuousSemigroup.complexOperator hS t))
    (t : ℝ) (x y : X) :
  ...
  have hxy : inner ℂ (S.toStronglyContinuousSemigroup.realOperator t x) y =
    ...
    rwa [ContinuousLinearMap.coe_coe, S.toStronglyContinuousSemigroup.complexOperator_apply hS,
      S.toStronglyContinuousSemigroup.complexOperator_apply hS,
