...
theorem isComplexLinear (hU : U.IsUnitary) : U.toSemigroup.IsComplexLinear := fun t z x => by
...
theorem complexGenerator_apply (U : StronglyContinuousGroup H) (hU : U.IsUnitary)
    ...
    U.complexGenerator hU x =
      U.generator ⟨(x : H), by
        ...
        exact x.property⟩ := by
  obtain ⟨x, hx⟩ := x
  exact (U.toSemigroup.complexGenerator_apply hU.isComplexLinear ⟨x, hx⟩).trans
    (@(LinearPMap.ext_iff.mp U.generator_def).2 x _ _).symm
